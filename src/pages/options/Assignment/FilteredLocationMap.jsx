  ...
  <div className="w-full h-[40vh]">
    <LoadScriptNext googleMapsApiKey={process.env.REACT_APP_MAPS_API_KEY}>
      <GoogleMap
        ...
        zoom={14}
      >
        {/* Render markers, only for the locations that match the clientId */}
        {Array.isArray(mapLocations) &&
          mapLocations
            .filter((location) => location.clientId === clientId) // Filter locations by clientId
            .map((location) => {
              ...
              const pinSVG = `
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="40" height="40" fill="${color}">
                  ...
                  <circle cx="12" cy="9" r="2.5" fill="#000000"/> <!-- Inner circle for glyph -->
                </svg>`;
