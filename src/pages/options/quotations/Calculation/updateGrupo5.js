const updateGrupo5 = (formData, valor3, allData) => {
  ...
  // Buscar la unidad de embarque según el nombre en `formData['Embarque'].nombre`
  const findEmbarqueUnidades = (nombreEmbarque) => {
    const itemEmbarque = allData.groups['Embarque'].items.find(
      item => item.nombre === nombreEmbarque
    );
    return itemEmbarque ? itemEmbarque.unidades : 0;
  };

  // Descripciones para las unidades
  const descriptions = {
    "Puerta_de_cabina": findEmbarqueUnidades(formData['Embarque'].nombre), // Buscar la unidad adecuada para Puerta de Cabina
    ...
    "Regulador_de_velocidad": formData['Regulador_de_velocidad'].UNIDADES || 0,
    "Freno": formData['Freno'].UNIDADES || 0,
  };
