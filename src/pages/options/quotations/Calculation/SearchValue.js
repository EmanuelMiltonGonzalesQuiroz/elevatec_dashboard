...
const SearchValue = (priceTableData, valorBuscado, campo) => {
  const priceTableItems = priceTableData["price table"]?.items || [];

  
  
  // Intentar encontrar el item utilizando areStringsSimilar
  let item = priceTableItems.find((item) => item.name === valorBuscado);
  // Si no se encuentra el item, intentar una búsqueda directa
  if (!item) {
    item =  priceTableItems.find((item) => areStringsSimilar(item.name, valorBuscado));
  }
