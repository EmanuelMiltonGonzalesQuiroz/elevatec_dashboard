...
const InfoModal = ({ isOpen, routeData, onClose }) => {
  ...
  const generateBuildingFields = (buildingName) => {
    if (['Oficinas Centricas', 'Oficinas Suburbios', 'Edificio empresarial', 'Edificio del gobierno'].includes(buildingName)) {
      return (
        <>
          <p><strong>Pisos:</strong> {buildingData.PISOS || additionalData.PISOS || 'N/A'}</p>
          <p><strong>Áreas:</strong> {additionalData[`piso_1_AREAS`] || 'N/A'}</p>
        </>
      );
      ...
      return (
        <>
          <p><strong>Pisos:</strong> {buildingData.PISOS || additionalData.PISOS || 'N/A'}</p>
          <p><strong>Departamentos:</strong> {additionalData[`piso_1_DEPARTAMENTOS`] || 'N/A'}</p>
        </>
      );
      ...
      return (
        <>
          <p><strong>Pisos:</strong> {buildingData.PISOS || additionalData.PISOS || 'N/A'}</p>
          <p><strong>Habitaciones:</strong> {additionalData[`piso_1_HABITACIONES`] || 'N/A'}</p>
        </>
      );
    } else if (['Hotel centrico', 'Hotel vacacional', 'Hotel economico', 'Hotel economico vacacional'].includes(buildingName)) {
      return (
        <>
          <p><strong>Pisos:</strong> {buildingData.PISOS || additionalData.PISOS || 'N/A'}</p>
          <p><strong>Habitaciones:</strong> {additionalData[`piso_1_HABITACIONES`] || 'N/A'}</p>
      ...
      return (
        <>
          <p><strong>Pisos:</strong> {buildingData.PISOS || additionalData.PISOS || 'N/A'}</p>
          <p><strong>Automóviles:</strong> {additionalData[`piso_1_AUTOMOVILES`] || 'N/A'}</p>
        </>
      );
    } else {
          ...
          <div className="mb-6 border-b pb-4">
            ...
            {resultData ? (
              <>
                ...
                <p><strong>Velocidad Desarrollada:</strong> {formatValue(resultData.Velocidad_Desarrollada)} m/s</p>
                <p><strong>Tiempo Total:</strong> {formatValue(resultData.Tiempo_total)} seg</p>
                <p><strong>Pasajeros Atendidos en 5 Minutos:</strong> {formatValue(resultData.Pasajeros_Atendidos_en_5_min)}</p>
                <p><strong>Cabinas Necesarias:</strong> {formatValue(resultData.Cabinas_Nesesarias)}</p>
                <p><strong>Calificación de Servicio:</strong> {resultData.Mensaje || 'No disponible'}</p>
              </>
            ) : (
